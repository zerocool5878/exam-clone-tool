...
def extract_comp_test_mapping_from_content(target_content, exam_content):
    ...
    Uses the corrected alternative-based matching algorithm
    """
    try:
        ...
        # Apply corrected alternative-based matching
        ...
        for i, (exam_q_num, exam_main_id) in enumerate(exam_sorted):
            ...
            if exam_main_id in target_main_ids:
                exam_to_target_mapping[exam_main_id] = exam_main_id  # Maps to itself
                print(f"DEBUG: Q{question_num} current ID {exam_main_id} is already a target main ID - suggest no change")
                continue
            ...
            if exam_section_match:
                ...
                # Check if any alternative matches a target main ID
                    ...
                    if alt_id in target_main_ids:
                        ...
                        for target_q_num, target_main_id in target_sorted:
                            if target_main_id == alt_id:
                                print(f"DEBUG: Exam Q{question_num} alternative {alt_id} matches target Q{target_q_num}")
                                break
                        break
                ...
                if matching_alternative:
                    ...
                    # NEW: Check if current exam ID appears as alternative in any target section
                    print(f"DEBUG: Q{question_num} - checking if current ID {exam_main_id} is target alternative...")
                    target_match = None
                    for j, (target_q_num, target_main_id) in enumerate(target_sorted):
                        # Extract target section to find its alternatives
                        if j < len(target_sorted) - 1:
                            next_target_q_num = int(target_sorted[j+1][0])
                            target_section_pattern = rf'{target_q_num}\.\s+.*?(?={next_target_q_num}\.\s+)'
                        else:
                            target_section_pattern = rf'{target_q_num}\.\s+.*'
                        
                        target_section_match = re.search(target_section_pattern, target_decoded, re.DOTALL)
                        if target_section_match:
                            target_section_content = target_section_match.group(0)
                            target_section_ids = re.findall(r'\(id:(\d+)\)', target_section_content)
                            
                            # Check if exam's current ID is in this target section as alternative
                            if exam_main_id in target_section_ids and exam_main_id != target_main_id:
                                target_match = target_main_id
                                print(f"DEBUG: Found! Exam Q{question_num} ID {exam_main_id} is alternative in target Q{target_q_num}")
                                break
                    
                    if target_match:
                        exam_to_target_mapping[exam_main_id] = target_match
                        print(f"DEBUG: Q{question_num} should change from {exam_main_id} -> {target_match}")
                    else:
